...
<div class="space-y-6">
	<!-- Header & Controls -->
	<div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">    <div>
    ...
    </div>    <Button on:click={openCreateModal} class="glass-button">
	...
	<div class="flex flex-wrap gap-4 items-center bg-accent/20 p-4 rounded-lg">
		<div class="relative flex-1 min-w-64">
			<Search class="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-muted-foreground" />      <input
        type="text"
      ...
      />
		...
		<select
			bind:value={filterRole}      class="px-3 py-2 border border-border rounded-lg bg-background text-foreground focus:border-primary focus:outline-none"
    >
      ...
      <option value="Masseuse">Masseuse</option>
		...
		<select
			bind:value={filterStatus}      class="px-3 py-2 border border-border rounded-lg bg-background text-foreground focus:border-primary focus:outline-none"
    >
	...
	<!-- Stats Cards -->
	<div class="grid grid-cols-1 md:grid-cols-4 gap-4">    <div class="enhanced-card p-4 rounded-lg text-center">
	...
	<div class="enhanced-card rounded-lg overflow-hidden">
		<div class="overflow-x-auto">
			<table class="w-full">        <thead class="bg-accent/50">
          <tr>
		...
		{#if filteredUsers.length === 0}      <div class="text-center py-12">
        ...
        <p class="text-muted-foreground">Try adjusting your filters to see results</p>
      </div>
		{/if}
	</div>
</div>
...
{#if showCreateModal}
	<div class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
		<div class="bg-background border border-border rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">      <div class="p-6 border-b border-border">
